...
- [ ] **WS-01**: 用户可以通过系统对话框选择本地文件夹作为工作区
- [ ] **WS-02**: 应用记录最近打开的工作区列表，支持快速切换
- [ ] **WS-03**: 切换工作区时自动清理旧 agent session 并初始化新 session
- [ ] **WS-04**: 应用启动时可自动恢复上次使用的工作区

### AI Chat

- [ ] **CHAT-01**: 用户可以在聊天面板中输入消息与 AI agent 对话
- [ ] **CHAT-02**: agent 回复流式实时显示（streaming）
- [ ] **CHAT-03**: 消息支持 Markdown 渲染（代码块、列表、表格等）
- [ ] **CHAT-04**: 对话历史可持久化到本地，重启后恢复
- [ ] **CHAT-05**: 用户可清空或新建对话

### Model Provider

- [ ] **PROV-01**: 用户可以添加 AI 模型提供商（至少支持 Anthropic、OpenAI）
- [ ] **PROV-02**: 用户可以为每个提供商配置 API Key
- [ ] **PROV-03**: 用户可以在可用模型中选择当前对话使用的模型
- [ ] **PROV-04**: 模型配置持久化到本地存储
- [ ] **PROV-05**: 连接失败时给出友好错误提示
