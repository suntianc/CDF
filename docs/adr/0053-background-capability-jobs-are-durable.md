...
Continuation execution is Conversation-scoped and may run while its Conversation is not visible. Main owns the run and durable output; renderer presence is never required. Main publishes session-scoped Conversation run events so a visible source Conversation can project the same live Agent stream as a foreground run. An invisible source Conversation remains in the background without changing the active Conversation, viewport, draft, or model controls; selecting it hydrates the current accumulated stream from main. The source Composer may reflect that its Conversation already has the one allowed active run, but its draft and model selection are preserved.
