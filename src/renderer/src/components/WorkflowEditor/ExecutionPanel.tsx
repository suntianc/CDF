...
export function ExecutionPanel({ executionId, taskGoal, onClose }: ExecutionPanelProps) {
  ...
  const { subscribeToExecution, nodeRuns, fetchNodeRuns, stopWorkflow, currentExecution, nodeTrace, pendingWorkflowApproval, resolveWorkflowApproval } = useWorkflowStore();
  ...
  return (
    <div className="w-[380px] bg-[var(--color-bg-sidebar)] border-l border-[var(--color-border)]/50 flex flex-col shrink-0 execution-panel-container">
      ...
      {pendingWorkflowApproval && (
        <div className="mx-4 mt-3 rounded-lg border border-[var(--color-warning)]/40 bg-[var(--color-bg-surface)] p-3 shadow-sm space-y-3">
          ...
          <div className="grid grid-cols-2 gap-2">
            <button
              type="button"
              className="rounded-md bg-[var(--color-accent)] px-3 py-2 text-xs font-medium text-white hover:opacity-90 transition-opacity"
            ...
            <button
              type="button"
              className="rounded-md border border-[var(--color-border)] px-3 py-2 text-xs font-medium text-[var(--color-danger)] hover:bg-[var(--color-bg-hover)] transition-colors"
