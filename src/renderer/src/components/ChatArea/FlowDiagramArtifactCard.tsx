  ...
  return (
    <div className="relative my-3 w-full max-w-[480px] rounded-[var(--radius-md)] border border-[var(--color-border)] bg-[var(--color-bg-surface)] transition-[border-color,background-color] duration-150 hover:border-[var(--color-border-strong)] hover:bg-[var(--color-bg-hover)]/30">
      ...
      >
        ...
        <span className="flex w-full items-center gap-3 border-t border-[var(--color-border)] px-4 py-3 pr-14">
          ...
          <span className="min-w-0 flex-1">
            ...
            <span
              className="mt-0.5 block truncate font-mono text-[11px] text-[var(--color-text-muted)]"
              title={sourcePath}
            >
              {sourcePath}
            </span>
      ...
      <Tooltip>
        ...
        <TooltipContent side="top">
          {copied ? t('chat.flowDiagramPathCopied') : `${t('chat.flowDiagramCopyPath')}: ${sourcePath}`}
        </TooltipContent>
      </Tooltip>
    </div>
  );
